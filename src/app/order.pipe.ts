...
export class OrderPipe implements PipeTransform {
  ...
  private sortArray(value: any[], expression?: any, reverse?: boolean, isCaseInsensitive?: boolean, comparator?: Function): any[] {
    ...
    if (reverse) {
      return this.setActiveCallOnTop(array.reverse(),reverse,isCaseInsensitive,compareFn);
    }
      
    return this.setActiveCallOnTop(array,reverse,isCaseInsensitive,compareFn);
  }
private setActiveCallOnTop= (array:any[],reverse,isCaseInsensitive,compareFn) =>{
   return array.filter(x=>x.isActiveCall == 1).concat(array.filter(x=>x.isActiveCall ==0));

//this.transform(array.filter(x=>x.isActiveCall == 1),'isActiveCall',reverse,isCaseInsensitive,compareFn)
  }
  ...
  private multiExpressionTransform(value: any, expressions: any[], reverse: boolean, isCaseInsensitive: boolean = false, comparator?: Function): any {
    return expressions.reverse().reduce((result: any, expression: any) => {
      return this.transform(result, expression, expression, isCaseInsensitive, comparator);
    }, value);
